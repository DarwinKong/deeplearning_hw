...
from source.agents.trainer import BaseTrainer, NEPTUNE_PROJECT_NAME
...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(self, env: Env, agent: PPOAgent, n_iter: int, n_games_train: int, agent_results_filepath: str,
        ...
        # Checkpoints callback - 自动保存最优模型到 remote
        checkpoint_callback = BestModelCheckpoint(
            dirpath=checkpoints_dir,
            remote_checkpoints_dir=remote_checkpoints_dir,
            filename='{epoch}_{step}',
            every_n_train_steps=100,
            verbose=True,
            monitor='train/reward',  # 监控训练奖励
            mode='max',  # 奖励越高越好
    ...
    def get_pl_logger(self, name: str, log_dir: str):
        ...
        """
        if name == "tensorboard":
            # 禁用 hyperparams 日志以避免 NumPy 2.0 兼容性问题
            return TensorBoardLogger(save_dir=log_dir, default_hp_metric=False)
        elif name == "neptune":
