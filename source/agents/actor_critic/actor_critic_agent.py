...
class ActorCriticAgent(BaseAgent):
    ...
    def _to_device(self, tensor: torch.Tensor) -> torch.Tensor:
        """将 tensor 移动到网络所在设备（CPU / CUDA / MPS）"""
    ...
    def get_value(self, state: np.ndarray) -> np.ndarray:
        ...
        """
        with torch.no_grad():
            return self.network.get_value(self._to_device(torch.from_numpy(state))).cpu().numpy()
    ...
    def get_policy(self, states: np.ndarray) -> np.ndarray:
        ...
        """
        with torch.no_grad():
            return self.network.get_policy(self._to_device(torch.from_numpy(states))).cpu().numpy()
