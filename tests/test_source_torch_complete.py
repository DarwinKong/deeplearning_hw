"""
测试 source-torch 完整实现
验证纯 PyTorch 版本的正确性和性能
"""
import sys
import os

# 添加项目根目录和 source-torch 到路径
project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, project_root)
sys.path.insert(0, os.path.join(project_root, 'source-torch'))

import torch
import time
from source_torch.env.env import Env
from source_torch.agents.actor_critic.actor_critic_agent import ActorCriticAgent
from source_torch.agents.ppo.ppo_agent import PPOAgent
...
def test_env_torch():
    ...
    env = Env(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    env.reset()
    ...
    assert state.device == env.device, f"State device mismatch: {state.device} vs {env.device}"
...
def test_agent_torch():
    ...
    network = FCPolicyValueNet(config).to(env.device)
...
def test_training_flow():
    """测试完整训练流程"""
    print("=" * 70)
    print("测试 3: 完整训练流程（2次迭代）")
    print("=" * 70)
    
    from source_torch.agents.actor_critic.actor_critic_trainer import ActorCriticTrainer
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        trainer = ActorCriticTrainer(
            ...
            n_iter=2,
            n_games_train=2,
            agent_results_filepath=os.path.join(tmpdir, "results.pkl"),
            n_steps_update=10,
            batch_size=32,
        ...
        print(f"✓ 平均速度: {2/elapsed:.2f} it/s")
...
if __name__ == "__main__":
    try:
        ...
        print("=" * 70)
        print("🎉 所有测试通过！Source-Torch 实现成功！")
        print("=" * 70)
        print("\n关键优化:")
