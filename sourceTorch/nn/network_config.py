...
class NetConfig:
    ...
    def _set_config_attributes(self, **kwargs):
        if self.config_dict is None:
            # 从 kwargs 设置配置（简化模式）
            self.name = kwargs.get('name', 'Net')
            self.activation_config = kwargs.get('activation', {'type': 'relu'})
            self.architecture_config = kwargs.get('architecture', {})
            self.loss_config = kwargs.get('loss', {})
            self.initializer_config = kwargs.get('initializer', {})
            self.optimizer_config = kwargs.get('optimizer', {})
        else:
            self._set_name()
